<?php 
  ...
  if(empty($_POST)){
    ...
    if (move_uploaded_file($_FILES['coverphoto']['tmp_name'], $targetcoverImg_folder)) {
      mysqli_query($con, "UPDATE admins SET cover_img_name = '$coverphoto_name' WHERE admin_id='$id'")or die(mysqli_error($con));
    }

    if (move_uploaded_file($_FILES['profilephoto']['tmp_name'], $targetprofileImg_folder)) {
      mysqli_query($con, "UPDATE admins SET profile_img_name = '$profilephoto_name' WHERE admin_id='$id'")or die(mysqli_error($con));
    }
